"""
Based on code written by Jude Markabawi (jmarkaba@andrew.cmu.edu)

Original code was shared by Jude in game1-sprite-based on the class Discord; this version
is only slightly modified to account for some variability in the color detection from load_image,
and to modify the tiles used to produce the background.
...
def find_tiles_and_palettes(img: np.ndarray):
    ...
    for y in range(0, bg_size[0], 8):
        for x in range(0, bg_size[1], 8):
            ...
            # Account for variability in load_image color detection
            for i in range(8):
                for j in range(8):
                    if tile[i][j][0] >= 200 and tile[i][j][2] >= 200:
                        tile[i][j] = np.array([0xff, 0xff, 0xff, 0xff])
                    elif tile[i][j][0] >= 200:
                        tile[i][j] = np.array([0xff, 0x00, 0x00, 0xff])
                    elif tile[i][j][2] >= 200:
                        tile[i][j] = np.array([0x00, 0x00, 0xff, 0xff])
                    else:
                        tile[i][j] = np.array([0x00, 0x00, 0x00, 0xff])
    ...
    for y in range(bg_tile_size[0]):
        for x in range(bg_tile_size[1]):
            tile = img[0:8, 0:8, :] # Set all background tiles to the first tile
            assert tuple(tile.shape) == (8, 8, 4)
